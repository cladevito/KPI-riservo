...
export default function Home() {
  ...
  // Il database può restituire le date come timestamp completi
  // (es. "2026-07-22T00:00:00.000Z"): le riportiamo sempre a "YYYY-MM-DD"
  // così confronti, raggruppamenti e formattazione non si rompono mai.
  ...
  const load = async () => {
    try {
      ...
      const raw: Call[] = await res.json();
      const data: Call[] = raw.map((c) => ({
        ...c,
        data_chiamata: normalizeDate(c.data_chiamata) ?? String(c.data_chiamata),
        data_demo: normalizeDate(c.data_demo),
        richiamare_il: normalizeDate(c.richiamare_il),
      }));
      setCalls(data);
      setError(null);
    } catch (e) {
